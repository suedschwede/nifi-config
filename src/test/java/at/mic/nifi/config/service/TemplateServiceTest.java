...
import at.mic.nifi.config.service.TemplateService;
...
public class TemplateServiceTest {
    ...
    public void installOnBranchTest() throws ApiException, IOException, URISyntaxException {
        ...
        when(processGroupsApiMock.uploadTemplate(anyString(), any(), true)).thenReturn(template);
        ...
        verify(processGroupsApiMock).uploadTemplate(response.getProcessGroupFlow().getId(), new File(fileName), true);
        verify(processGroupsApiMock).instantiateTemplate(response.getProcessGroupFlow().getId(), instantiateTemplate);
        verify(templatesApiMock, never()).removeTemplate(template.getTemplate().getId(), true);
    }
    ...
    public void installOnBranchWithRemoveTemplateTest() throws ApiException, IOException, URISyntaxException {
        ...
        verify(processGroupsApiMock).uploadTemplate(response.getProcessGroupFlow().getId(), new File(fileName), true);
        verify(processGroupsApiMock).instantiateTemplate(response.getProcessGroupFlow().getId(), instantiateTemplate);
        verify(templatesApiMock).removeTemplate(template.getTemplate().getId(), true);
    }
    ...
    public void undeployTest() throws ApiException {
        ...
        when(processGroupsApiMock.removeProcessGroup("idProcessGroupFlow","10", null, true)).thenReturn(new ProcessGroupEntity());
        ...
        verify(templatesApiMock).removeTemplate(template.getId(), true);
