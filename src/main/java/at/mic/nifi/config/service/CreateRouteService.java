...
import at.mic.nifi.config.model.ConnectionPort;
...
public class CreateRouteService {
  ...
  private void startPorts(Stream<PortEntity> connectables) {
    connectables.map(port -> {
		try {
			return portService.getById(port.getId(), port.getComponent().getType());
		} catch (ApiException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
			return null;
 ...
 * @throws ApiException 
   */
  public void createRoutes(List<ConnectionPort> connections, boolean optionNoStartProcessors) throws IOException, ApiException {
